...
function renderUser() {
  ...
  if (ranked.length===0 || ranked.every(p=>p.total===0)) {
    ...
    const SHOW_LIMIT = 10;
    const showAll = filtered._showAll || searchQuery.length > 0;
    ...
    lb.innerHTML = visible.map((p,i)=>{
      ...
      ? `<div style="text-align:center;padding:14px 0 4px;">
           <button onclick="document.querySelector('#u-leaderboard')._showAll=true;renderUser();"
