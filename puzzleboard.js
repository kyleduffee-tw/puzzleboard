...
const SUPABASE_KEY = 'sb_publishable_G4qthDxApENsg2bZr9GODw_3dLoQpdD';
...
async function storageGet() {
  try {
    ...
    _supabaseOk = false;
    try { return localStorage.getItem(STORE_KEY); } catch(e2) { return null; }
  }
}
...
async function loadState() {
  try {
    ...
    if (raw && raw !== '{}') S = { ...DEFAULTS, ...JSON.parse(raw) };
  } catch(e) {}
  _storageReady = true;
...
async function save() {
  try { await storageSet(JSON.stringify(S)); } catch(e) {}
}
...
function adminSaveSettings() {
  ...
  // Default period — resolve "current" tokens to real values
  const rawPeriod = document.getElementById('a-default-period')?.value || '7d';
  S.defaultPeriod = resolveDefaultPeriod(rawPeriod);

  save(); renderAdmin();
  showToast('Settings saved');
}

// Resolve "current" period tokens to concrete values at save time
...
function adminPasteImport() {
  ...
  added.forEach(p => {
    [p.sender, p.recipient].forEach(name => {
      const SKIP_NAMES = new Set(['Team','Unknown','PuzzleBoard','Recognition']);
  ...
  reader.onload = function(e) {
    try {
      ...
      rows.forEach(row => {
        ...
        const note = msgText.replace(PUZZLE_RE, '').replace(/@\S+/g, '').trim().slice(0, 80);
        const fp   = makeFp(sender || 'Unknown', recipient, dateStr, note);
        allParsed.push({ sender: sender || 'Unknown', recipient, count, date: dateStr, note, fp });
      });

      if (!allParsed.length) { showToast('No 🧩 found in spreadsheet.', true); return; }
...
function parsePasted(rawText) {
  ...
  for (const rawLine of lines) {
    ...
    const SKIP_RECIPIENTS = new Set(['Team','Unknown','PuzzleBoard','Recognition']);
