...
async function loadState() {
  ...
  renderUser(); // re-render so loading state updates to real data
}
...
function renderUser() {
  ...
  if (ranked.length===0 || ranked.every(p=>p.total===0)) {
    lb.innerHTML = _storageReady
      ? `<div class="empty-state"><div class="empty-emoji">🧩</div><div class="empty-title">No puzzle pieces yet</div><div class="empty-sub">Click "+ Log 🧩" to give your first shoutout.</div></div>`
      : `<div class="empty-state"><div class="empty-emoji" style="font-size:32px;">⏳</div><div class="empty-title">Loading…</div><div class="empty-sub">Fetching board data</div></div>`;
  } else {
