...
function showSyncErrorModal(hasCachedData = true) {
  ...
  const subMsg = hasCachedData
    ? 'Showing your last cached data.<br>Check your connection and click ↻ to try again.'
    : 'No data is available right now.<br>You can attempt to reload the data or try again later.';
...
function renderUser() {
  ...
  if (ranked.length===0 || ranked.every(p=>p.total===0)) {
    ...
    if (!_storageReady) {
      // Still loading
      emptyHTML = `<div class="empty-state"><div class="empty-emoji" style="font-size:32px;">⏳</div><div class="empty-title">Loading…</div><div class="empty-sub">Fetching board data</div></div>`;
    } else if (_lastRefreshFailed) {
      // Failed to load — no cache, no data
      emptyHTML = `<div class="empty-state"><div class="empty-emoji" style="font-size:32px;">⚠️</div><div class="empty-title">Board not loaded</div><div class="empty-sub">Could not reach the server. Click ↻ to try syncing again.</div></div>`;
    } else if (S.members.length === 0) {
      // Connected but genuinely empty — new installation
      emptyHTML = `<div class="empty-state"><div class="empty-emoji">🧩</div><div class="empty-title">No puzzle pieces yet</div><div class="empty-sub">Recognitions will appear here once pieces start flowing.</div></div>`;
    } else {
      // Has members but no entries in this period
      emptyHTML = `<div class="empty-state"><div class="empty-emoji">🧩</div><div class="empty-title">No puzzle pieces yet</div><div class="empty-sub">No recognitions logged for this period yet.</div></div>`;
    }
