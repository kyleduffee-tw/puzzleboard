...
const DEFAULTS = {
  ...
  defaultSort: 'received', // 'sent' | 'received'
  sort: 'received',
...
function showSyncErrorModal(hasCachedData = true) {
  ...
  modal.innerHTML = `
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:32px 28px;max-width:340px;width:90%;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,0.18);">
      <img src="images/mascot_sad.png" alt="" style="width:72px;height:auto;margin-bottom:12px;">
...
function renderUser() {
  ...
  if (ranked.length===0 || ranked.every(p=>p.total===0)) {
    ...
    if (!_storageReady) {
      // Still loading
      emptyHTML = `<div class="empty-state"><img src="images/mascot_happy.png" alt="" style="width:64px;height:auto;animation:bounce 1s ease-in-out infinite;margin-bottom:8px;"><div class="empty-title">Loading…</div><div class="empty-sub">Fetching board data</div></div>`;
    } else if (_lastRefreshFailed) {
      // Failed to load — no cache, no data
      emptyHTML = `<div class="empty-state"><img src="images/mascot_sad.png" alt="" style="width:64px;height:auto;margin-bottom:8px;"><div class="empty-title">Board not loaded</div><div class="empty-sub">PuzzleBoard couldn't reach its data server. This usually resolves on its own — try ↻ again in a moment.</div></div>`;
    } else if (S.members.length === 0) {
      // Connected but genuinely empty — new installation
      emptyHTML = `<div class="empty-state"><img src="images/mascot_happy.png" alt="" style="width:64px;height:auto;margin-bottom:8px;"><div class="empty-title">No puzzle pieces yet</div><div class="empty-sub">Recognitions will appear here once pieces start flowing.</div></div>`;
    } else {
      // Has members but no entries in this period
      emptyHTML = `<div class="empty-state"><img src="images/mascot_happy.png" alt="" style="width:64px;height:auto;margin-bottom:8px;"><div class="empty-title">No puzzle pieces yet</div><div class="empty-sub">No recognitions logged for this period yet.</div></div>`;
    }
    ...
    lb.innerHTML = visible.map((p,i)=>{
      ...
      const sortVal   = S.sort==='sent'?p.sent:p.received;
      const cbClass   = S.sort==='sent'?'cb-sent':'cb-recv';
      const sortLbl   = S.sort==='sent'?'sent':'recv';
...
function switchAdminTab(tab) {
  _adminTab = tab;
  ['people','settings','audit'].forEach(t => {
...
function requireSupabase() {
  if (_supabaseOk) return true;
  showSyncErrorModal(true);
  // Override the modal message for this specific case
  const msgEl = document.querySelector('#syncErrorModal div[style*="font-size:13px"]');
  if (msgEl) msgEl.innerHTML = 'PuzzleBoard is not connected to its data server right now.<br>Importing while disconnected could result in lost data. Please wait until the connection is restored and try again.';
  return false;
}
...
function renderPickerGrid(type) {
  ...
  if (type === 'quarter') {
    ...
    grid.innerHTML = QUARTERS.map((q, i) => {
      ...
      const isActive = S.period === val;
      const qStart   = (_pickerYear * 100) + (i * 3) + 1; // YYYYMM of first month of quarter
      const nowYM    = (now.getFullYear() * 100) + (now.getMonth() + 1);
      const isFuture = qStart > nowYM; // disable only if quarter hasn't started yet
      return `<button class="popover-item ${isActive ? 'active' : ''}"
        ${isFuture ? 'disabled style="opacity:0.3;cursor:not-allowed;"' : ''}
...
function updatePeriodLabel() {
  ...
  // Sync active state — clear all first then set the correct one
  document.querySelectorAll('#u-period-group .pill').forEach(b => b.classList.remove('active'));
  if (p === '7d')                document.getElementById('pp-7d')?.classList.add('active');
  else if (p === '30d')          document.getElementById('pp-30d')?.classList.add('active');
  else if (p === 'all')          document.getElementById('pp-all')?.classList.add('active');
  else if (p.startsWith('month:'))   document.getElementById('pp-month')?.classList.add('active');
  else if (p.startsWith('quarter:')) document.getElementById('pp-quarter')?.classList.add('active');
...
function renderBannerDrawer(tab) {
  ...
  el.innerHTML = '<div class="drawer-entries-scroll">' + entries.map(e => {
...
function renderAuditLog() {
  ...
  const entries = [...S.entries]
    ...
    .filter(e => {
      if (filter === 'bonus'   && !e.isBonus)  return false;
      if (filter === 'regular' &&  e.isBonus)  return false;
      if (search) {
        const hay = (e.sender + ' ' + e.recipient + ' ' + (e.note||'')).toLowerCase();
  ...
  if (!entries.length) {
    body.innerHTML = `<tr><td colspan="5" style="padding:16px;text-align:center;color:var(--muted);">No entries found</td></tr>`;
    if (footer) footer.textContent = '0 entries';
    return;
  }

  const overLimitSet = buildOverLimitSet();

  body.innerHTML = entries.map((e, i) => {
    const bg = i % 2 === 0 ? '' : 'background:var(--surface2);';
    const senderColor = e.isBonus ? 'color:var(--accent);font-weight:700;' : '';
    const isOver = overLimitSet.has(e.fp);
    const overBadge = isOver ? '<span style="font-size:9px;color:var(--warn);margin-left:4px;font-weight:700;">⚠ over limit</span>' : '';
    const rowStyle = isOver ? `${bg}opacity:0.7;` : bg;
    const noteText = e.note ? `<span style="color:var(--muted);font-style:italic;">"${e.note.slice(0,60)}${e.note.length>60?'…':''}"</span>` : '—';
    return `<tr style="${rowStyle}">
      <td style="padding:6px 10px;color:var(--muted);white-space:nowrap;">${fmtDate(e.date)}</td>
      <td style="padding:6px 10px;${senderColor}">${e.sender}${overBadge}</td>
      <td style="padding:6px 10px;font-weight:600;color:var(--text);">${e.recipient}</td>
      <td style="padding:6px 10px;text-align:center;">${'🧩'.repeat(Math.min(e.count,5))}${e.count>5?`<span style="font-size:10px">+${e.count-5}</span>`:''}</td>
      <td style="padding:6px 10px;">${noteText}</td>
    </tr>`;
  }).join('');

  if (footer) footer.textContent = `${entries.length} entr${entries.length!==1?'ies':'y'} · ${S.entries.length} total`;
}
...
// Like yearsSince but calculates as of a specific YYYY-MM-DD check date
function yearsSinceDate(dateStr, asOf) {
  if (!dateStr || !asOf) return -1;
  const hire  = parseLocalDate(dateStr);
  const check = parseLocalDate(asOf);
  if (!hire || !check) return -1;
  let years = check.getFullYear() - hire.getFullYear();
  const annivThisYear = new Date(check.getFullYear(), hire.getMonth(), hire.getDate());
  if (annivThisYear > check) years--;
  return years;
}
...
function alreadyAwarded(name, type, year) {
  // Key on calendar year so the lookback window doesn't re-award across days
  const currentYear = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Chicago' }).split('-')[0];
  return (S.awardedMilestones || []).some(a =>
    a.name === name && a.type === type && a.year === year &&
    (a.calYear === currentYear || a.date === today()) // support legacy entries keyed by date
  );
}
...
function recordAward(name, type, year) {
  ...
  const currentYear = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Chicago' }).split('-')[0];
  S.awardedMilestones.push({ name, type, year, date: today(), calYear: currentYear });
}

// Award bonus pieces as a system entry
// eventDate: the actual birthday/anniversary date (may differ from today with lookback)
function awardMilestoneBonus(name, count, note, eventDate) {
  const awardDate = eventDate || today();
  const fp = makeFp('Patch', name, awardDate, note);
  // Check fp dedup too
  if (S.entries.some(e => e.fp === fp)) return;
  S.entries.push({ sender: 'Patch', recipient: name, count, date: awardDate, note, fp, isBonus: true, isAuto: true });
}
...
function processMilestones() {
  ...
  // Build a list of dates to check — today plus up to 5 days back
  // This catches weekends and long weekends where nobody opened the app
  const datesToCheck = [];
  for (let i = 0; i <= 5; i++) {
    const d = new Date();
    d.setDate(d.getDate() - i);
    datesToCheck.push(d.toLocaleDateString('en-CA', { timeZone: 'America/Chicago' }));
  }
  ...
  S.members.forEach(m => {
    ...
    if (S.birthdayEnabled && m.birthday) {
      ...
      if (bd) {
        for (const checkDate of datesToCheck) {
          const [yr, mo, dy] = checkDate.split('-').map(Number);
          if (mo === bd.month && dy === bd.day && !alreadyAwarded(m.name, 'birthday', 0)) {
            awardMilestoneBonus(m.name, S.birthdayBonus, `🎂 Birthday bonus!`, checkDate);
    ...
    if (S.anniversaryEnabled && m.hireDate) {
      for (const checkDate of datesToCheck) {
        const yearsOnDate = yearsSinceDate(m.hireDate, checkDate);
        const [yr, mo, dy] = checkDate.split('-').map(Number);
        const hd = m.hireDate.split('-');
        const hireMonth = parseInt(hd[1]);
        const hireDay   = parseInt(hd[2]);
        if (mo === hireMonth && dy === hireDay && yearsOnDate >= 1 && !alreadyAwarded(m.name, 'anniversary', yearsOnDate)) {
          const isMilestone = MILESTONE_YEARS.includes(yearsOnDate);
          const bonus = S.anniversaryBonus + (isMilestone ? S.milestoneBonus : 0);
          const note  = isMilestone
            ? `🏆 ${yearsOnDate}-Year Milestone bonus!`
            : `🎉 ${yearsOnDate}-Year Work Anniversary bonus!`;
          awardMilestoneBonus(m.name, bonus, note, checkDate);
          recordAward(m.name, 'anniversary', yearsOnDate);
          changed = true;
          break; // only award once
        }
      }
...
function renderMilestoneCard(item) {
  ...
  const msImg = item.type === 'birthday'
    ? 'images/birthday_mascot.png'
    : 'images/milestone_mascot.png';
...
function buildSpotlightEntries() {
  ...
  const withNote = capped
    .filter(e => e.note && e.note.trim() && !e.isAuto)
    .map((e, i) => ({...e, _idx: i}))
    .sort((a,b) => b.date.localeCompare(a.date) || b._idx - a._idx);

  // Try 7 days first — if fewer than 3 entries fall back to 30 days, then all-time
  const cutoffs = [7, 30, 9999];
  for (const days of cutoffs) {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - days);
    const cutoffStr = cutoff.toLocaleDateString('en-CA', { timeZone: 'America/Chicago' });
    const filtered = days === 9999 ? withNote : withNote.filter(e => e.date >= cutoffStr);
    if (filtered.length >= 3 || days === 9999) return filtered.slice(0, 20);
  }
  return [];
}
  ...
  setTimeout(() => {
    ...
    if (metaEl) {
      metaEl.innerHTML = e.isBonus
        ? `<img src="images/celebration_mascot.png" style="width:14px;height:auto;vertical-align:middle;margin-right:4px;">Patch → <strong>${e.recipient}</strong>`
        : `From <strong>${e.sender}</strong> → <strong>${e.recipient}</strong> · ${(function(d){
            const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
            const parts = d.split('-');
            return months[parseInt(parts[1])-1] + ' ' + parseInt(parts[2]) + ', ' + parts[0];
          })(e.date)}`;
    }
    if (mascot) mascot.src = e.isBonus ? 'images/celebration_mascot.png' : 'images/mascot_happy.png';
    // Fade in
...
(async () => {
  ...
  // seedDemo disabled — real data only
