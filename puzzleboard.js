...
async function loadState(forceRefresh = false) {
  ...
  // Only mark as failed if this was a forced/manual refresh, not the initial background load
  const markFailure = forceRefresh;
  try {
  ...
  _lastRefreshFailed = markFailure && !supabaseOk;
  console.log('[PuzzleBoard] loadState done — supabaseOk:', supabaseOk, 'markFailure:', markFailure, '_lastRefreshFailed:', _lastRefreshFailed, 'cacheTimestamp:', S.cacheTimestamp);
  _storageReady = true;
  requestAnimationFrame(updateStorageStatus);
  renderUser();
  updateLastUpdatedLabel();
...
function updateLastUpdatedLabel() {
  ...
  if (!el) return;
  // Always check failure flag first — overrides any cached timestamp
...
function renderUser() {
  ...
  if (ranked.length===0 || ranked.every(p=>p.total===0)) {
    lb.innerHTML = _storageReady
      ...
      : _lastRefreshFailed
        ? `<div class="empty-state"><div class="empty-emoji" style="font-size:32px;">⚠️</div><div class="empty-title">Couldn't reach the server</div><div class="empty-sub">No cached data available. Check your connection and try the ↻ button to attempt the connection again.</div></div>`
