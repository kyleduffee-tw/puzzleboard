...
function updateLastUpdatedLabel() {
  ...
  if (!el) return;
  if (!S.cacheTimestamp) { el.textContent = ''; return; }
...
async function forceRefresh() {
  ...
  if (!ok) {
    // Show failure state in overlay for 3 seconds before dismissing
    ...
    if (overlayText) overlayText.innerHTML =
      ...
      '<span style="font-weight:400;opacity:0.75;">Showing your last cached data — try again in a moment.</span>';
...
function normBirthday(raw) {
  ...
  if (raw instanceof Date) return `${String(raw.getMonth()+1).padStart(2,'0')}-${String(raw.getDate()).padStart(2,'0')}`;
...
function normHireDate(raw) {
  if (!raw) return null;
  // Handle JavaScript Date objects (XLSX library with cellDates:true)
  if (raw instanceof Date) return raw.toISOString().split('T')[0];
  const s = String(raw).trim();
  if (!s) return null;
  // ISO string with time component e.g. '2019-03-15T00:00:00.000Z'
