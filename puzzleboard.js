...
let _lbShowAll = false; // leaderboard show-all state
...
function renderUser() {
  ...
  if (ranked.length===0 || ranked.every(p=>p.total===0)) {
    ...
    const SHOW_LIMIT = 10;
    const showAll = _lbShowAll || searchQuery.length > 0;
    ...
    lb.innerHTML = visible.map((p,i)=>{
      ...
      ? `<div style="text-align:center;padding:14px 0 4px;">
           <button onclick="_lbShowAll=true;renderUser();"
             style="background:none;border:1px solid var(--border);border-radius:8px;padding:8px 20px;color:var(--muted);font-size:12px;font-family:'Inter',sans-serif;cursor:pointer;">
...
function openPatchBonusModal() {
  ...
  const sel = document.getElementById('pb-recipient');
  ...
  setTimeout(() => {
    ...
    if (metaEl) {
      metaEl.innerHTML = e.isBonus
        ? `<img src="mascot_happy.png" style="width:14px;height:auto;vertical-align:middle;margin-right:4px;">Patch → <strong>${e.recipient}</strong>`
        : `From <strong>${e.sender}</strong> → <strong>${e.recipient}</strong> · ${e.date}`;
    }
    if (mascot) mascot.src = e.isBonus ? 'mascot_happy.png' : 'mascot_happy.png';
    // Fade in
